...
export default function ReactiveSpectrum({analyser,audio,mode,color,background=false,gain=1,particleCount=100,seed=1}:{analyser:RefObject<AnalyserNode|null>;audio:RefObject<HTMLAudioElement|null>;mode:string;color:string;background?:boolean;gain?:number;particleCount?:number;seed?:number}){
 ...
 useEffect(()=>{
  ...
  const draw=(now:number)=>{
   ...
   travel+=dt*energy;
   const css=getComputedStyle(canvas.parentElement!);const particleColor=css.getPropertyValue('--particle-color').trim()||color;const baseSize=Number(css.getPropertyValue('--particle-base-size'))||.55;const glow=Number(css.getPropertyValue('--particle-glow'))!==0;const wind=css.getPropertyValue('--particle-wind').trim();const wx=wind==='left'?-1:wind==='right'?1:0,wy=wind==='up'?-1:wind==='down'?1:0;const renderColor=background?particleColor:color;ctx.strokeStyle=renderColor;ctx.fillStyle=renderColor;ctx.lineWidth=1.4;ctx.shadowColor=renderColor;ctx.shadowBlur=glow?3+high*18:0;
   ...
   if(background||['universe','star-field','particles','dot-field'].includes(mode)){
    const points=particles.map((p,i)=>{if(active){const local=smooth[i%96]||energy;p.x=(p.x+(p.vx*dt*(.35+reactive*4)+wx*dt*(.08+reactive*.35))+Math.sin(travel*2+p.phase)*dt*reactive*.09+1)%1;p.y=(p.y+(p.vy*dt*(.35+reactive*4)+wy*dt*(.08+reactive*.35))+Math.cos(travel*1.7+p.phase)*dt*reactive*.09+1)%1}const pulse=1+smooth[i%96]*1.6+reactive*.7;const x=p.x*w,y=p.y*h;return [x+(x-w/2)*bass*.16,y+(y-h/2)*bass*.16,p.z*pulse*baseSize]});
