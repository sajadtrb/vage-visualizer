 ...
 useEffect(()=>{
  ...
  const draw=(now:number)=>{
   ...
   if(background||['universe','star-field','particles','dot-field'].includes(mode)){
    ...
    points.filter((_,i)=>i%4===0).forEach(([x,y,z],i)=>{const v=smooth[(i*11)%96]||0;ctx.globalAlpha=.04+reactive*.2+v*.25;ctx.shadowBlur=glow?(5+reactive*16)*baseSize:0;dot(x,y,1.15*z+reactive*.9*z)});ctx.shadowBlur=glow?(3+high*18)*baseSize:0;ctx.globalAlpha=1;
